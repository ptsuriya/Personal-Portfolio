...
export default function RbruSkillArticle() {
  return (
    <main id="content" className="mx-auto w-full max-w-6xl px-4 pb-20 sm:px-6 lg:pb-28">
      <header className="border-b border-kuma-line pt-10 pb-12 sm:pt-14 sm:pb-16">
        <p className="text-[15px] text-kuma-amber-deep">KUMA notes / agent workflow</p>
        <h1 className="mt-3 max-w-4xl text-4xl font-bold leading-[1.1] tracking-[-0.025em] text-kuma-bark sm:text-6xl">Claude / Codex Config<br />Skills และ MCP ใช้ยังไง?</h1>
          <p className="mt-6 max-w-2xl text-lg leading-8 text-kuma-clay">คู่มือรวม Skill และ MCP สำหรับงาน Laravel, React, UI/UX, planning, PDF, browser QA และ repository ตั้งแต่เลือกใช้ ติดตั้ง ไปจนถึงเขียน prompt ให้ agent ทำงานได้ตรงจุด</p>
          <div className="mt-8 flex flex-wrap gap-2 text-sm text-kuma-cocoa"><span className="rounded-lg border border-kuma-line bg-white/50 px-3 py-1.5">Skills 20 รายการ</span><span className="rounded-lg border border-kuma-line bg-white/50 px-3 py-1.5">MCP 9 รายการ</span><span className="rounded-lg border border-kuma-line bg-white/50 px-3 py-1.5">อัปเดต 22 ก.ค. 2026</span></div>
      </header>
      ...
      <div className="mt-12 grid gap-10 lg:grid-cols-[220px_minmax(0,1fr)] lg:items-start lg:gap-16">
        <aside className="lg:sticky lg:top-28">
          ...
          <nav aria-label="สารบัญบทความ" className="mt-3 grid border-l border-kuma-line text-[15px]">
            <a href="#start" className="-ml-px border-l-2 border-transparent px-4 py-2 text-kuma-clay hover:border-kuma-amber hover:text-kuma-bark">เริ่มต้น: Skill vs MCP</a>
            <a href="#using" className="-ml-px border-l-2 border-transparent px-4 py-2 text-kuma-clay hover:border-kuma-amber hover:text-kuma-bark">วิธีเรียกใช้</a>
            <a href="#install-playwright" className="-ml-px border-l-2 border-transparent py-1.5 pl-7 pr-4 text-sm text-kuma-clay hover:border-kuma-amber hover:text-kuma-bark">ติดตั้ง Playwright ทั้งชุด</a>
            <a href="#agent-instructions" className="-ml-px border-l-2 border-transparent py-1.5 pl-7 pr-4 text-sm text-kuma-clay hover:border-kuma-amber hover:text-kuma-bark">Codex / Claude instructions</a>
            <a href="#skills" className="-ml-px border-l-2 border-transparent px-4 py-2 text-kuma-clay hover:border-kuma-amber hover:text-kuma-bark">Skills</a>
            {skillGroups.map((group) => <a key={group.id} href={`#${group.id}`} className="-ml-px border-l-2 border-transparent py-1.5 pl-7 pr-4 text-sm text-kuma-clay hover:border-kuma-amber hover:text-kuma-bark">{group.title}</a>)}
            <a href="#mcps" className="-ml-px border-l-2 border-transparent px-4 py-2 text-kuma-clay hover:border-kuma-amber hover:text-kuma-bark">MCP</a>
            <a href="#workflow" className="-ml-px border-l-2 border-transparent px-4 py-2 text-kuma-clay hover:border-kuma-amber hover:text-kuma-bark">Workflow แนะนำ</a>
          </nav>
        </aside>
